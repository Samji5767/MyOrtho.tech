...
import type { CreatePrintJobDto, JobStatus } from './manufacturing.dto';
...
export class ManufacturingService {
  ...
  async createJob(
    ...
    const { rows } = await this.pool.query(
      `INSERT INTO print_jobs
         (organization_id, printer_id, stage_id, gcode_path, status, qc_notes)
       VALUES ($1, $2, $3, $4, 'queued', $5)
       RETURNING id, status, created_at`,
      [orgId, dto.printerId ?? null, dto.stageId ?? null, dto.gcodePath ?? null, dto.qcNotes ?? null],
    );
    const job = rows[0];
  ...
  async updateJobStatus(
    ...
    const { rows } = await this.pool.query(
      ...
      [status, startedAt, completedAt, id, orgId],
    );
    if (!rows[0]) throw new NotFoundException('Print job not found');
  ...
  async listPrinters(orgId: string) {
    const { rows } = await this.pool.query(
      `SELECT id, name, brand, model, status, ip_address, firmware_version,
              material_type, material_volume_ml, created_at, updated_at
       FROM printers
