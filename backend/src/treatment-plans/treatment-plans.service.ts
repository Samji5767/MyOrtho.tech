...
export class TreatmentPlansService {
  ...
  /**
   * Pre-approval validation. Every check runs against persisted data — the
   * same rules approvePlan enforces, surfaced ahead of time so the UI can
   * explain exactly why approval is or is not possible. Advisory warnings
   * flag movements beyond common review thresholds; they are review guidance,
   * not clinical validation.
   */
  async validateApproval(planId: string, caseId: string, orgId: string) {
    await this.verifyCaseOwnership(caseId, orgId);

    const { rows: planRows } = await this.pool.query(
      `SELECT id, doctor_approval FROM treatment_plans WHERE id = $1 AND case_id = $2`,
      [planId, caseId],
    );
    if (!planRows[0]) throw new NotFoundException('Treatment plan not found');
    const alreadyApproved = planRows[0]['doctor_approval'] as boolean;

    const { rows: stageStats } = await this.pool.query(
      `SELECT count(*)::int AS total,
              count(*) FILTER (
                WHERE (movement_data::jsonb ->> '_is_simulated')::boolean = true
              )::int AS simulated
       FROM aligner_stages WHERE treatment_plan_id = $1`,
      [planId],
    );
    const stageCount = stageStats[0]['total'] as number;
    const simulatedCount = stageStats[0]['simulated'] as number;

    // Review thresholds for clinician-entered movements (cumulative totals).
    const { rows: largeRows } = await this.pool.query(
      `SELECT DISTINCT tm.fdi_number
       FROM tooth_movements tm
       JOIN aligner_stages ast ON ast.id = tm.stage_id
       WHERE ast.treatment_plan_id = $1
         AND (abs(tm.mesiodistal_mm) > 4 OR abs(tm.buccolingual_mm) > 4
              OR abs(tm.occlusogingival_mm) > 2 OR abs(tm.rotation_deg) > 30
              OR abs(tm.tip_deg) > 15 OR abs(tm.torque_deg) > 15)
       ORDER BY tm.fdi_number`,
      [planId],
    );

    const blockers: Array<{ code: string; description: string; affectedTeeth?: number[] }> = [];
    const warnings: Array<{ code: string; description: string; affectedTeeth?: number[] }> = [];

    if (alreadyApproved) {
      blockers.push({
        code: 'ALREADY_APPROVED',
        description: 'This treatment plan has already been approved and is locked.',
      });
    }
    if (stageCount === 0) {
      blockers.push({
        code: 'NO_STAGES',
        description: 'The plan has no aligner stages. Add stages before approval.',
      });
    }
    if (simulatedCount > 0) {
      blockers.push({
        ...
        description:
          `${simulatedCount} of ${stageCount} stages were generated by the rule-based ` +
          'scaffold and are not clinical data. Replace them before approval.',
      });
    }
    if (largeRows.length > 0) {
      warnings.push({
        code: 'LARGE_MOVEMENT',
        description:
          'Some teeth have prescribed movements beyond common review thresholds ' +
          '(>4 mm translation, >2 mm vertical, >30° rotation, >15° tip/torque). ' +
          'Verify these are intended.',
        affectedTeeth: largeRows.map((r) => r['fdi_number'] as number),
      });
    }

    const score = Math.max(0, Math.min(100, 100 - blockers.length * 40 - warnings.length * 10));
    const canApprove = blockers.length === 0;
    const summary = canApprove
      ? `${stageCount} stage${stageCount === 1 ? '' : 's'} ready for doctor approval.`
