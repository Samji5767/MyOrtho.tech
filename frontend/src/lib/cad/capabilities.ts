...
export const CAD_CAPABILITIES: CadCapability[] = [
  ...
  },
  {
    ...
    phase: "plan",
    maturity: "implemented",
    summary: "12-axis movement prescriptions per tooth: translation (mesial/distal/buccal/lingual), intrusion, extrusion, rotation, torque, mesial/distal tip, mesialization, distalization, expansion, constriction, root movement. Per-tooth inline limit warnings against Kravitz thresholds.",
    statusNote: "Fully wired in Phase 26: ToothMovementEditor.tsx → /api/cases/:id/plans/:id/movements/prescriptions. FDI color grid shows movement magnitude. Approve-all workflow. Prescriptions stored in movement_prescriptions table with UNIQUE(plan_id, tooth_number).",
    surface: "/treatment-plan",
  },
  {
    id: "movement-simulation",
    name: "Movement Simulation Engine",
    phase: "plan",
    maturity: "implemented",
    summary: "Full biomechanical simulation: interproximal collision detection, biological limit violation scoring (Kravitz), anchorage classification (maximum/moderate/minimum, Proffit units), bone remodeling index, estimated stage count. Stage count = max required stages across all prescribed movements.",
    statusNote: "Fully wired in Phase 26: MovementSimulationPanel.tsx → POST /api/cases/:id/plans/:id/movements/simulate. Results stored in movement_simulations (UNIQUE plan_id). Re-run overwrites. Simulation in <100ms.",
    surface: "/treatment-plan",
  },
  {
    id: "pdl-simulation",
    name: "PDL Stress Simulation",
    phase: "plan",
    maturity: "implemented",
    summary: "Per-tooth per-stage PDL stress (MPa), strain (%), applied force (N), and moment (N·cm) using published PDL area averages (Melsen 2001) by tooth type. Mobility risk classification: none / low / moderate / high based on stress thresholds (Yoshida 2001).",
    statusNote: "Fully wired in Phase 26: pdl_simulation_results table with per-stage data. Stored at stages 1, every 5th, and final. PdlStageViewer scrubs across stages with a slider.",
    surface: "/treatment-plan",
  },
  {
